...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/qotu.sn-iriga`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `qotu.sn-iriga` and import [qotu.sn-iriga.sql](qotu.sn-iriga.sql) into it.

6. Open the terminal and navigate to the project directory **qotu.sn-iriga**.
...
9. Open your web browser and access <http://localhost:5182/qotu.sn-iriga/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-qotu.sn-iriga` and import [qotu.sn-iriga.sql](qotu.sn-iriga.sql) into it.
...
2. Access the application by visiting `http://[host_name]/qotu.sn-iriga`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/qotu.sn-iriga>
     - <http://192.168.1.99/qotu.sn-iriga>
